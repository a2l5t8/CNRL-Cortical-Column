<<<<<<< Updated upstream:src/L5.6/neuron/GPCell.py
import torch
...
class GPCell(LIF):
    ...
    def initialize(self, neurons):
        ...
        neurons.v = neurons.v.to(device=neurons.network.device)
        neurons.spike_prev = neurons.vector("zeros") < 0
    ...
    def forward(self, neurons):
        if(torch.isnan(neurons._v[0])): 
            ...
            return
        newPosX = (
            neurons.x[neurons.spikes]
            + (self.shape[1] - 1) / 2
            + neurons._v[0] * self.L
        )

        newPosY = (     
            neurons.y[neurons.spikes]
            + (self.shape[2] - 1) / 2
            + neurons._v[1] * self.L
        )
...
>>>>>>> Stashed changes:src/L56/neuron/GPCell.py
