...
class AttentionBasedRSTDP(SimpleRSTDP) : 
    ...
    def forward2(self, synapse) :
        ...
        attention_mat = att.expand((synapse.src.size, -1)).to(synapse.device)
        """ --------- """
    ...
    def forward(self, synapse) :
        ...
        att_value = 0
        if(dec == -1 or idx == dec) : 
            att_value = self.attention_plus
        else : 
            att_value = self.attention_minus

        synapse.c += (-synapse.c / self.tau_c) + computed_stdp * att_value
        synapse.weights += synapse.c * synapse.network.dopamine_concentration